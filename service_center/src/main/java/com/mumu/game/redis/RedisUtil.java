...
import com.mumu.game.redis.manager.RedisExpireManager;
...
public class RedisUtil {
    ...
    /** 执行redis操作 */
    public static <T> T doCmd(String handler, T defValue, Function<StringRedisTemplate, T> func, Object... params) {
        try {
    ...
    public static boolean tryLock(String key, String val, int expire) {
        try {
            Boolean res = stringRedisTemplate.opsForValue().setIfAbsent(key, val, expire, TimeUnit.SECONDS);
    ...
    /** 批量获取多个key的值 */
    public static <T> Map<String, T> batchGet(String keyPattern, int eachCount, Function<String, T> func) {
    ...
    public static void publish(String channel, byte[] message) {
        stringRedisTemplate.execute(connection -> connection.publish(channel.getBytes(StandardCharsets.UTF_8), message),
            true);
    }
    ...
    public static void setBit(String key, long offset, boolean value) {
        doCmd("setBit", StringUtils.EMPTY, t -> t.opsForValue().setBit(key, offset, value), key, offset, value);
    }
    ...
    public static <K, V> Map<K, V> hmget(String key, Function<String, K> kFunc, Function<String, V> vFunc) {
        try {
            Map<Object, Object> entries = stringRedisTemplate.opsForHash().entries(key);
            if (CollUtil.isEmpty(entries))
                return Collections.emptyMap();
            return entries.entrySet().stream().collect(
                Collectors.toMap(e -> kFunc.apply(e.getKey().toString()), e -> vFunc.apply(e.getValue().toString())));
        } catch (Exception e) {
    ...
    public static <V> List<V> batchHGetValues(String key, String fieldPattern, int eachCount,
        Function<String, V> vFunc) {
        return Lists.newArrayList(batchHGet(key, fieldPattern, eachCount, k -> k, vFunc).values());
    }

    /** 批量获取 hashKey的多个值（分页获取） */
    public static <K, V> Map<K, V> batchHGet(String key, String fieldPattern, int eachCount, Function<String, K> kFunc,
        Function<String, V> vFunc) {
        try (RedisConnection conn = factory.getConnection()) {
            ...
            try (Cursor<Map.Entry<byte[], byte[]>> cursor =
                ...
                while (cursor.hasNext()) {
                    ...
                    resMap.put(kFunc.apply(new String(entry.getKey(), StandardCharsets.UTF_8)),
                        vFunc.apply(new String(entry.getValue(), StandardCharsets.UTF_8)));
                }
                return resMap;
            }
        } catch (Exception e) {
            LogTopic.ACTION.error(e, "batchGetKey", "key", key, "fieldPattern", fieldPattern, "eachCount", eachCount);
            return Collections.emptyMap();
        }
    }
    ...
    public static void batchHSet(String key, Map<String, String> map, int eachCount) {
        partition(map, eachCount).forEach(eachMap -> hmset(key, eachMap));
    }
    ...
    public static <K, V> List<Map<K, V>> partition(Map<K, V> map, int size) {
        ...
        return Lists.partition(Lists.newArrayList(map.entrySet()), size).stream()
                .map(eachList -> CollStreamUtil.toMap(eachList, Map.Entry::getKey, Map.Entry::getValue))
                .toList();
    }
    ...
    public static boolean sHasKey(String key, Object value) {
        return doCmd("sHasKey", false, tmp -> tmp.opsForSet().isMember(key, String.valueOf(value)), key, value);
    }

    /** 取出并移除set中N个元素 */
    public static List<String> sPop(String key, long count) {
        return doCmd("sPop", Collections.emptyList(), tmp -> tmp.opsForSet().pop(key, count), key, count);
    }
    ...
    public static long getRank(String key, long id) {
        return doCmd("getRank", -1L,
            t -> Optional.ofNullable(t.opsForZSet().reverseRank(key, String.valueOf(id))).map(r -> r + 1).orElse(-1L),
            key, id);
    }
    ...
    public static boolean zadd(String key, long id, long score) {
        return doCmd("zadd", false, t -> t.opsForZSet().add(key, String.valueOf(id), score), key, id, score);
    }
    ...
    public static long zIncr(String key, long id, long score, boolean removeNegative) {
        ...
        // id,
        // score);
        List<String> keys = List.of(key, String.valueOf(id), String.valueOf(score),
                String.valueOf(System.currentTimeMillis() / 1000), String.valueOf(removeNegative));
        return RedisLuaScript.ZSCORE_UPDATE_BY_TIME.executeLua(keys);
    }
    ...
    /** 增加玩家积分 removeNegative-true，表示积分小于0时移除 */
    public static void incrWithExpire(String key, long id, long score, int expireTime, boolean removeNegative) {
        executePipelined(connection -> {
            RedisLuaScript.ZSCORE_UPDATE_BY_TIME.executeLua(connection, key, String.valueOf(id), String.valueOf(score),
                String.valueOf(System.currentTimeMillis() / 1000), String.valueOf(removeNegative));
            if (expireTime > 0)
                RedisExpireManager.expire(key, expireTime, connection::expire);
        });
    }
    ...
    public static Set<String> getElementsAboveScore(String key, long score) {
        return doCmd("rangeByScore", Collections.emptySet(),
            t -> t.opsForZSet().rangeByScore(key, score, Double.POSITIVE_INFINITY), key, score);
    }

    /** 移除指定积分范围的元素 */
    public static long removeRangeByScore(String key, long min, long max) {
        return doCmd("removeRangeByScore", 0L, t -> t.opsForZSet().removeRangeByScore(key, min, max), key, min, max);
    }

    /** 获取排名 TopN （rank-排名，从1开始，0-返回全部） */
    public static Set<ZSetOperations.TypedTuple<String>> getTopN(String key, int rank) {
        return doCmd("getTopN", Collections.emptySet(), t -> t.opsForZSet().reverseRangeWithScores(key, 0, rank - 1),
            key, rank);
    }
