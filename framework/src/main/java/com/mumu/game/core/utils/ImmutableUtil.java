...
public class ImmutableUtil {
     ...
     * @since 2024/10/29 18:24
     */
    public static <K, V> ImmutableMap<K, ImmutableList<V>> list2ImmMapWithList(Collection<V> list,
        Function<V, K> convert) {
        if (list == null || convert == null) {
            return ImmutableMap.of();
        }
        ...
        for (V v : list) {
            ...
            ImmutableList.Builder<V> listBuilder = tmpMap.computeIfAbsent(mainKey, o -> ImmutableList.builder());
            listBuilder.add(v);
        }
     ...
     * @param convert 提供一个key
     * @return com.google.common.collect.ImmutableMap<K,com.google.common.collect.ImmutableList<V>>
     * @since 2024/10/29 18:28
     */
    public static <K, V> ImmutableMap<K, ImmutableList<V>> list2ImmMapWithMultiKeyList(Collection<V> list,
        Function<V, List<K>> convert) {
        if (list == null || convert == null) {
            return ImmutableMap.of();
        }
     ...
     * @since 2024/10/29 20:07
     */
    public static <K1, V, K2, K3> ImmutableMap<K1, ImmutableMap<K2, ImmutableMap<K3, V>>> list2ImmMapWithSubSubMap(
        Collection<V> list, Function<V, K1> convert, Function<V, K2> convert2, Function<V, K3> convert3) {
        if (list == null || convert == null) {
            return ImmutableMap.of();
        }
     ...
     * @since 2024/10/29 20:19
     */
    public static <K1, K2, V> ImmutableMap<K1, ImmutableMap<K2, ImmutableList<V>>>
        list2ImmMapWithSubMapListMap(Collection<V> list, Function<V, K1> convert, Function<V, K2> convert2) {
        if (list == null || convert == null) {
            return ImmutableMap.of();
        }
    ...
    public static void main(String[] args) {
        ...
        // 1. list2ImmMap
        ImmutableMap<Integer, ImmutableOb> key1ObMap = ImmutableUtil.list2ImmMap(list, ImmutableOb::getKey1);
        System.out.println("1. list2ImmMap() -------- key1ObMap: " + key1ObMap);

        // 2. list2MultiKeyImmMap
        ImmutableMap<Integer, ImmutableOb> keysObMap = ImmutableUtil.list2MultiKeyImmMap(list, ImmutableOb::getKeys);
        System.out.println("2. list2MultiKeyImmMap -------- keysObMap: " + keysObMap);

        // 3. list2ImmMapWithList
        ImmutableMap<Integer, ImmutableList<ImmutableOb>> typeObsMap =
            ImmutableUtil.list2ImmMapWithList(list, ImmutableOb::getType);
        System.out.println("3. list2ImmMapWithList -------- typeObsMap: " + typeObsMap);

        // 4. list2ImmMapWithMultiKeyList
        ImmutableMap<Integer, ImmutableList<ImmutableOb>> multiKeysObsMap =
            ImmutableUtil.list2ImmMapWithMultiKeyList(list, ImmutableOb::getKeys);
        System.out.println("4. list2ImmMapWithMultiKeyList -------- multiKeysObsMap: " + multiKeysObsMap);

        // 5. list2ImmMapWithSubMap
        ImmutableMap<Integer, ImmutableMap<Integer, ImmutableOb>> key1Key2ObMap =
            ImmutableUtil.list2ImmMapWithSubMap(list, ImmutableOb::getKey1, ImmutableOb::getKey2);
        System.out.println("5. list2ImmMapWithSubMap -------- key1Key2ObMap: " + key1Key2ObMap);

        // 6. list2ImmMapWithSubSubMap
        ImmutableMap<Integer, ImmutableMap<Integer, ImmutableMap<Integer, ImmutableOb>>> key1Key2Key3ObMap =
            ImmutableUtil.list2ImmMapWithSubSubMap(list, ImmutableOb::getKey1, ImmutableOb::getKey2,
                ImmutableOb::getKey3);
        System.out.println("6. list2ImmMapWithSubSubMap -------- key1Key2Key3ObMap: " + key1Key2Key3ObMap);

        // 7. list2ImmMapWithSubMapListMap
        ImmutableMap<Integer, ImmutableMap<Integer, ImmutableList<ImmutableOb>>> typeKey1ListMap =
            ImmutableUtil.list2ImmMapWithSubMapListMap(list, ImmutableOb::getType, ImmutableOb::getKey1);
