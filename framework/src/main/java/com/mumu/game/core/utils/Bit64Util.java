...
public final class Bit64Util {
    ...
    public static String giMapToStr(Map<Integer, Long> stateMap) {
        ...
        for (int key : keys) {
            ...
            if (key != pre + 1) {
                sb.append(Symbol.SEMICOLON).append(key).append(Symbol.COLON).append(state);
            } else {
        ...
        // 删除第一个分号
        if (sb.indexOf(Symbol.SEMICOLON) == 0)
            sb.delete(0, 1);
        return sb.toString();
    }
    ...
    public static Map<Integer, Long> strTogiMap(String giStateMapStr) {
        ...
        if (StringUtils.isEmpty(giStateMapStr))
            return map;
