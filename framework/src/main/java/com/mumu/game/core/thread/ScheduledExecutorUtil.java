...
public class ScheduledExecutorUtil {
    ...
    public static ScheduledFuture<?> scheduleWithFixedDelay(String key, Runnable command, long initialDelay, long delay,
        TimeUnit unit) {
        return scheduledFutureMap.computeIfAbsent(key,
            k -> scheduledExecutor.scheduleWithFixedDelay(wrapper(key, command, false), initialDelay, delay, unit));
    }

    /** 以固定速率执行，不会因为任务耗时，导致下次任务推辞执行 */
    public static ScheduledFuture<?> scheduleAtFixedRate(String key, Runnable command, long initialDelay, long delay,
        TimeUnit unit) {
        return scheduledFutureMap.computeIfAbsent(key,
            k -> scheduledExecutor.scheduleAtFixedRate(wrapper(key, command, false), initialDelay, delay, unit));
    }
    ...
    /** 注册一个延迟任务 delay: 延迟时间 */
    public static ScheduledFuture<?> schedule(String key, Runnable command, long delay, TimeUnit unit) {
    ...
    public static ScheduledFuture<?> schedule(String key, Runnable command, long delay, TimeUnit unit,
        ...
        if (isReplace)
            cancel(key);
        return scheduledFutureMap.computeIfAbsent(key,
            k -> scheduledExecutor.schedule(wrapper(key, command), delay, unit));
    }
