...
public class CovertUtil {
    ...
    public static <T, R extends Collection<T>> R stringToCollection(String value, String split, Class<T> type,
        Supplier<R> supplier) {
        return stringToCollection(value, split, parseTpyeFunMap.get(type), supplier);
    }

    /** String -> Collection 不会返回null（注：supplier不能传null） */
    public static <T, R extends Collection<T>> R stringToCollection(String value, String split,
        Function<String, ?> func, Supplier<R> supplier) {
    ...
    public static <K, V> String pairToString(Pair<K, V> pair, String split, Function<K, String> keyFunc,
        Function<V, String> valFunc) {
        if (pair == null)
    ...
    public static <K, V> List<Pair<K, V>> stringToPairList(String value, String subSplit, String split,
        Class<K> keyType, Class<V> valueType) {
        return stringToCollection(value, split, v -> stringToPair(v, subSplit, keyType, valueType),
            Lists::newArrayList);
    }

    public static <K, V> Pair<K, V> stringToPair(String value, String split, Class<K> keyType, Class<V> valueType) {
        return stringToPair(value, split, parseTpyeFunMap.get(keyType), parseTpyeFunMap.get(valueType));
    }

    public static <K, V> Pair<K, V> stringToPair(String value, String split, Function<String, ?> funcKey,
        Function<String, ?> funcValue) {
    ...
    public static <K, V> Map<K, V> stringToMap(String value, String subSplit, String split, Class<K> kType,
        Class<V> vType) {
    ...
    @SuppressWarnings("unchecked")
    public static <K, V> Map<K, V> stringToMap(String value, String subSplit, String split, Class<K> kType,
        Class<V> vType, Supplier<Map<K, V>> mapSupplier) {
        return stringToMap(value, subSplit, split, (Function<String, K>)parseTpyeFunMap.get(kType),
            (Function<String, V>)parseTpyeFunMap.get(vType), mapSupplier);
    }

    public static <K, V> Map<K, V> stringToMap(String value, String subSplit, String split, Function<String, K> kFun,
    ...
    @SuppressWarnings("unchecked")
    public static <K, V> Map<K, V> stringToMap(String value, String subSplit, String split, Function<String, K> kFun,
        Function<String, V> vFun, Supplier<Map<K, V>> mapSupplier) {
        if (mapSupplier == null) {
    ...
    /** collection --> String 不会反回null */
    public static <T> String collToString(Collection<T> collection, String split, Function<T, String> func) {
     ...
     * @return 不会反回 null
     */
    public static <K, V> String mapToString(Map<K, V> map, String subSplit, String split, Function<K, String> kFun,
        Function<V, String> vFun) {
        if (MapUtil.isEmpty(map) || isBlank(subSplit, split) || kFun == null || vFun == null) {
            return StringUtils.EMPTY;
        }
        StringBuilder sb = new StringBuilder();
        map.forEach((k, v) -> sb.append(kFun.apply(k)).append(subSplit).append(vFun.apply(v)).append(split));
    ...
    public static <K, V> List<Map<K, V>> partition(Map<K, V> map, int size) {
        ...
        return Lists.partition(Lists.newArrayList(map.entrySet()), size).stream()
            .map(eachList -> CollStreamUtil.toMap(eachList, Entry::getKey, Entry::getValue)).toList();
    }
}
