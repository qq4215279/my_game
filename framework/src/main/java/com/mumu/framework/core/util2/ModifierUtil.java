...
public class ModifierUtil extends cn.hutool.core.util.ModifierUtil {
    ...
    public static <T extends S, S> Class<T> getGenericInterfaceClass(Class<?> clazz, Class<?> rawType,
        Class<S> superClass) {
    ...
    /** 查找实现类上父类中指定类型的泛型声名 */
    public static <T extends S, S> Class<T> getGenericSuperClass(Class<?> clazz, Class<S> superClass) {
        return getGenericClass(clazz.getGenericSuperclass(), null, superClass);
    }

    /** 查找指定类上指定指定类型的泛型声名 */
    public static <T extends S, S> Class<T> getGenericClass(Type genType, Class<?> rawType, Class<S> superClass) {
        if (genType instanceof ParameterizedType paramType) {
            if (rawType == null || rawType.isAssignableFrom((Class<?>)paramType.getRawType())) {
                for (Type type : paramType.getActualTypeArguments()) {
                    if (type instanceof Class client && (superClass == null || superClass.isAssignableFrom(client))) {
                        return client;
                    }
                }
            }
    ...
    public static void checkDuplicateField(Class<?> clazz) {
        ...
        for (Field f : clazz.getDeclaredFields()) {
            ...
            Assert.isFalse(map.containsKey(value), "发现重复的属性值[{}]！k1: {}, K2: {}", value, name, map.get(value));
            map.put(value, name);
        }
    }
}
